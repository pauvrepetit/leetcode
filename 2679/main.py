...
class Solution:
    def matrixSum(self, nums: List[List[int]]) -> int:
        trans_nums = [[] for _ in range(len(nums[0]))]
        for num in nums:
            num.sort(reverse=True)
            for i in range(len(num)):
                trans_nums[i].append(num[i])
        max_sum = 0
